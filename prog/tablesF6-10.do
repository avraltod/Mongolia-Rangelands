...
********************************************************************************
*TABLE F.6-10: Heterogeneous Estimated Marginal Effects (herd density)
********************************************************************************	
	use "$dta/wrk/wrk_soum.dta" , clear 
		...
		* june survey in sheep units (t, calender year )
		cap drop survey 
			g survey = log(su_d1_svy_total)
				la var survey "$ \ln HD ^{June}_{st} $"
		* december census in sheep units (t+1, non calender year)
		cap drop census 
		g census = log(su_d1_cen_total)  
		la var census "$ \ln  HD ^{December}_{st} $"
	...
	save "$dta/wrk/wrk_soum_hd_eco.dta" , replace 
*------------------------------------------------------------------------------*
**# Bootstrapping  
*------------------------------------------------------------------------------*
	cap program drop BSHDECO 
	program define BSHDECO, rclass
	syntax [, lag(integer 1) fst(integer 2) sst(integer 2) ]
	
	* bootstrapping 
	use "$dta/wrk/wrk_soum_hd_eco.dta" , clear 
	tempfile boot 
	save `boot', replace 
...
// 			saving("$dta/wrk/boot_hd_eco.dta" , replace): BSHDECO , lag(1) fst(9) sst(40)
*------------------------------------------------------------------------------*	
	use "$dta/wrk/boot_hd_eco.dta", clear 
		g id=_n
	...
	save "$dta/wrk/sd_hd_eco.dta", replace
*------------------------------------------------------------------------------*	
// 	* SEs without lag
// 	simulate  ///
// 		hshat_ez11 = r(b11) hshat_ez12 = r(b21) hshat_ez13 = r(b31) hshat_ez14 = r(b41) hshat_ez15 = r(b51) hshat_ez16 = r(b61) ///
...
// 		/* lndvilag1 = r(b111) lndvilag2 = r(b211) lndvilag3 = r(b311) lndvilag4 = r(b411) lndvilag5 = r(b511) lndvilag6 = r(b611) */ ///
// 			, reps(10000) /* nodots nolegend */ seed(0123456789) ///
// 			saving("$dta/wrk/boot_hd_eco_nolag.dta" , replace): BSHDECONoLag , lag(0) fst(9) sst(40)	
*------------------------------------------------------------------------------*
	use "$dta/wrk/boot_hd_eco_nolag.dta" , clear 
		g id=_n
		...
		collapse (mean) `M'  (sd) `SD' , by(col)	
		table () (col), stat(mean m_* ) stat(mean sd_* ) nototals nformat(%9.4f)
	save "$dta/wrk/sd_hd_eco_nolag.dta", replace
*------------------------------------------------------------------------------*

********************************************************************************
********************************************************************************
**#TABLE F.6: OLS: Regressing log of soum-level SGR-based average NDVI on the log June
*density interacted with ecological zone dummies, GDD(>20) interacted with ecological zone
	...
	quietly esttab OLS_* using "$olf/tab/supp/`tex'.tex", replace substitute(\_ _) ///
			...
			, label(\multicolumn{1}{c}{Y} \multicolumn{1}{c}{N})) ///
						refcat( ///
							...
							survey_ez1 "\it $ \ln HD ^{June}_{st, idw} \, \times $ Eco." ///
							sgr_smr_hed5_ez1 " $ GDD(>20^\circ\text{C}) \, \times $ \it Eco." ///
...
********************************************************************************
**#TABLE F.7: OLS: Regressing log of soum-level SGR-based average NDVI on the log December
*herd density interacted with ecological zone dummies, GDD(>20) interacted with ecological zone
*dummies, SGR-based summer weather variables in levels (temperature bins, precipitation,
*wind), and other covariates.
********************************************************************************
*------------------------------------------------------------------------------*
	...
	quietly esttab OLS_* using "$olf/tab/supp/`tex'.tex", replace substitute(\_ _) ///
			...
			, label(\multicolumn{1}{c}{Y} \multicolumn{1}{c}{N})) ///
						refcat( ///
							...
							sgr_wtr_hed3 "\it  Winter weather, SGR" ///
							sgr_smr_hed3 "\it  Summer weather, SGR" ///
							census_ez1 "\it $ \ln HD ^{Dec}_{st, idw} \, \times $ Eco." ///
							sgr_smr_hed5_ez1 " $ GDD(>20^\circ\text{C}) \, \times $ \it Eco." ///
							wgr_aut_ced3 "\it  Fall weather, WGR" ///
							wgr_spr_ced3 "\it  Spring weather, WGR" ///
							wgr_wtr_ced3 "\it  Winter weather, WGR" ///
...
********************************************************************************
**#Table F.8: First Stage: Results from equation (C.1) regressing the log of June density on
*the log of December herd density interacted with ecological zone dummies, GDD(>20) interacted
...
*------------------------------------------------------------------------------*
	...
	esttab FS_* using "$olf/tab/supp/`tex'.tex", replace substitute(\_ _) ///
			...
			booktabs alignment(D{.}{.}{-1}) fragment ///
			nomtitles  ///
			mgroups("(a) Dependent variable: $ \ln  HD ^{June}_{st,idw} $ " , pattern(1 0 0 0 0) ///
			prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
			...
			, label(\multicolumn{1}{c}{Y} \multicolumn{1}{c}{N})) ///
				refcat( ///
					...
					census_ez1 "\it $ \ln HD ^{December}_{st} \, \times $ Eco." ///					
					, nolabel) 	
			

********************************************************************************
**#Table F.9: Second Stage: Results from equation (C.2) regressing log of soum-level SGR-
*based average NDVI on the log of predicted June herd density interacted with ecological zone
...
*------------------------------------------------------------------------------*	
	...
	cap drop hshat 
	g hshat = . 
	la var hshat "$ \widehat{HD}^{June}_{st.idw}$"
	...
	*bringing bootstrapping	
	preserve 
		use "$dta/wrk/bse/sd_hd_eco.dta", clear 
		matrix S = J(6,11,.)
	...
	local tex "eco5_sls_ndvi_wLag_bse"
	 esttab SLS_* using "$olf/tab/supp/`tex'.tex", replace substitute(\_ _) ///
			...
			, label(\multicolumn{1}{c}{Y} \multicolumn{1}{c}{N})) ///
						refcat( ///
							...
							sgr_wtr_hed3 "\it  Winter weather, SGR" ///
							sgr_smr_hed3 "\it  Summer weather, SGR" ///
							hshat_ez1 "\it $ \ln \widehat{HD} ^{June}_{st, iwd} \, \times $ Eco." ///
							sgr_smr_hed5_ez1 "$ GDD(>20^\circ\text{C}) \, \times $ \it Eco." ///
							, nolabel) 
...
********************************************************************************
**# Table F.10: Second Stage: Results from equation (C.2) regressing log of soum-level SGR-
* based average NDVI on the log of predicted June herd density interacted with ecological zone
* dummies, GDD(>20) interacted with ecological zone dummies, SGR-based summer weather
...
*------------------------------------------------------------------------------*	
	...
	*bringing bootstrapping	
	preserve 
		use "$dta/wrk/bse/sd_hd_eco_nolag.dta", clear 
			matrix S = J(6,10,.)
			matrix rownames S = se 
			matrix colnames S = ${sumvar} 					
			matrix list S 
