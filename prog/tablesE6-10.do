// =============================================================================
// Filename:  do10_reg2_hd_20241208.do
// Author:     Avralt-Od Purevjav
// Last Modified:  Avraa
// Date:     2024-12-08
// =============================================================================
// Marginal Effects of Climate and Herd Density (Annex E)
// ========================================================================== //
	* 
	clear all
	set more off
...
********************************************************************************
*TABLE E.6-E10: Estimated Marginal Effects of Climate and Herd Density 
********************************************************************************	
	use "$dta/wrk/wrk_soum.dta", clear 
		keep year asid su_*_total *na_ave sgr_* wgr_* eco* 
	...
	save "$dta/wrk/wrk_soum_hd.dta" , replace 
	
*------------------------------------------------------------------------------*
**# bootstrapping SEs 
*------------------------------------------------------------------------------*
	
	cap program drop BSHD 
	program define BSHD, rclass
	syntax [, lag(integer 1) fst(integer 2) sst(integer 3) ]
	
		* bootstrapping 
		use "$dta/wrk/wrk_soum_hd.dta" , clear 
		tempfile boot 
		save `boot', replace 
...
// 			saving($dta/wrk/boot_hd.dta , replace): BSHD , lag(0) fst(9) sst(40)
*------------------------------------------------------------------------------*
	use "$dta/wrk/boot_hd.dta" , clear 
		g id=_n
	...
	save "$dta/wrk/sd_hd.dta", replace
*------------------------------------------------------------------------------*	
	* SEs without lag 
// 	simulate  ///
// 		hshat1 = r(b11) hshat2 = r(b21) hshat3 = r(b31) hshat4 = r(b41) hshat5 = r(b51) hshat6 = r(b61) ///
...
*------------------------------------------------------------------------------*
	use "$dta/wrk/boot_hd_nolag.dta" , clear 
		...
		collapse (mean) `M'  (sd) `SD' , by(col)	
		table () (col), stat(mean m_* ) stat(mean sd_* ) nototals nformat(%9.4f)
	save "$dta/wrk/sd_hd_nolag.dta", replace
*------------------------------------------------------------------------------*

********************************************************************************
********************************************************************************
*TABLE E.6: OLS: Regressing log of soum-level SGR-based average NDVI on the log of June
* herd density, SGR-based summer weather variables in levels (temperature bins, precipitation,
...
********************************************************************************
*TABLE E.7: OLS: Regressing log of soum-level SGR-based average NDVI on the log of Decem-
* ber herd density, SGR-based summer weather variables in levels (temperature bins, precipitation,
* wind), and other covariates.
********************************************************************************
*------------------------------------------------------------------------------*
* OLS with Dec Census Density 
*------------------------------------------------------------------------------*
	use "$dta/wrk/wrk_soum_hd.dta" , clear 
	xtset asid year 
...
********************************************************************************
*Table E.8: First Stage: Results from equation (C.1) regressing the log of June herd density on the
*log of December herd density, SGR-based summer weather variables in levels (temperature bins,
...
*------------------------------------------------------------------------------*
	...
	local tex "idw_ols_hshat"
	esttab FS_* using "$olf/tab/supp/`tex'.tex", replace substitute(\_ _) ///
			cells(b(fmt(3) star) se(par fmt(3)) ) /// //se(par fmt(3))
			star(* 0.10 ** 0.05 *** 0.01) ///
			label compress nogaps nodepvars collabels(none) nomtitles ///
			booktabs alignment(D{.}{.}{-1}) fragment ///
			mgroups("(a) Dependent variable: $ \ln  HD ^{June}_{st} $ " , pattern(1 0 0 0 0) ///
			prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
...
********************************************************************************
*Table E.8: Second Stage: Results from equation (C.2) regressing log of soum-level SGR-
*based average NDVI on the log of predicted June herd density, SGR-based summer weather
...
*------------------------------------------------------------------------------*	
	...
	cap drop hshat 
	g hshat = . 
	la var hshat "$ \ln \widehat{HD}^{June}_{st}$"
	...
	*bringing bootstrapping
	preserve 
		use "$dta/wrk/bse/sd_hd.dta", clear 
			matrix S = J(6,7,.)
	...
	local tex "idw_sls_ndvi_shapley4_wLag_bse"
	 esttab SLS_* using "$olf/tab/supp/`tex'.tex", replace substitute(\_ _) ///
			...
			refcat( ///
				...
				, nolabel) 	
********************************************************************************
* Table E.10: Second Stage: Results from equation (C.2) regressing log of soum-level SGR-
*based average NDVI on the log of predicted June herd density, SGR-based summer weather
...
*------------------------------------------------------------------------------*	
	...
	*bringing bootstrapping
	preserve 
		use "$dta/wrk/bse/sd_hd_nolag.dta", clear 
			matrix S = J(6,6,.)
			matrix rownames S = se se se se se se
			matrix colnames S = ${sumvar} 					
			matrix list S 
	...
	local tex "idw_sls_ndvi_shapley4_bse"
	 esttab SLS_* using "$olf/tab/supp/`tex'.tex", replace substitute(\_ _) ///
			cells(b(fmt(3) star) SD(par fmt(3) pvalue(PV)) ) /// //se(par fmt(3))
			star(* 0.10 ** 0.05 *** 0.01) ///
			label compress nogaps nodepvars collabels(none) nomtitles ///
